...
// Fix potential default Leaflet asset lookup issues
      ...
      ">
        ${
          ...
          <div style="
            ...
            inset: -6px;
            border-radius: 18px;
            border: 2.5px solid ${pulseRingColor};
            pointer-events: none;
...
const activeMachineIcon = createProfessionalIcon('linear-gradient(135deg, #0284C7 0%, #0369A1 100%)', SVG_ICONS.tractor, 38, true, '#0284c7');
const availableMachineIcon = createProfessionalIcon('linear-gradient(135deg, #10B981 0%, #059669 100%)', SVG_ICONS.tractor, 32);
const maintenanceIcon = createProfessionalIcon('linear-gradient(135deg, #E11D48 0%, #BE123C 100%)', SVG_ICONS.alert, 34, true, '#e11d48');
...
/**
 * Handles map sizing and container invalidation on mount / tab change
 */
...
export const LeafletFleetMap: React.FC<LeafletFleetMapProps> = ({
  ...
  if (!isMounted) {
    return (
      <div
        style={{ height }}
        className="w-full bg-slate-100 rounded-2xl flex items-center justify-center text-slate-400 font-medium text-xs"
      >
  ...
  return (
    <div
      style={{ height }}
      className="w-full rounded-2xl overflow-hidden border border-slate-200 shadow-elevated relative z-0"
    >
          ...
          {showLayerMenu && (
            <div className="absolute right-0 mt-1.5 w-64 bg-white rounded-2xl border border-slate-200 shadow-2xl p-2 z-[1010] space-y-1">
              ...
              <button
                type="button"
                onClick={() => {
                  setBaseLayer('CARTO_VOYAGER');
                  setShowLayerMenu(false);
                }}
                className={clsx(
                  'w-full text-left px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all cursor-pointer',
                  baseLayer === 'CARTO_VOYAGER'
                    ? 'bg-emerald-50 text-emerald-950 border border-emerald-200'
                    : 'text-slate-700 hover:bg-slate-100'
                )}
              >
              ...
              <button
                type="button"
                onClick={() => {
                  setBaseLayer('OSM_STANDARD');
                  setShowLayerMenu(false);
                }}
                className={clsx(
                  'w-full text-left px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all cursor-pointer',
                  baseLayer === 'OSM_STANDARD'
                    ? 'bg-emerald-50 text-emerald-950 border border-emerald-200'
                    : 'text-slate-700 hover:bg-slate-100'
                )}
              >
              ...
              <button
                type="button"
                onClick={() => {
                  setBaseLayer('DARK_TELEMATICS');
                  setShowLayerMenu(false);
                }}
                className={clsx(
                  'w-full text-left px-3 py-2 rounded-xl text-xs font-bold flex items-center gap-2 transition-all cursor-pointer',
                  baseLayer === 'DARK_TELEMATICS'
                    ? 'bg-emerald-50 text-emerald-950 border border-emerald-200'
                    : 'text-slate-700 hover:bg-slate-100'
                )}
              >
            ...
            <Popup>
              <div className="text-xs p-1 max-w-[200px]">
                ...
                <div className="text-slate-500 font-medium">
                  {farm.sizeAcres} Acres • {farm.crop.cropName}
                </div>
                <div className="text-[10px] text-emerald-700 font-bold mt-1">
                  Stage: {farm.crop.cropStage}
                </div>
              </div>
            </Popup>
          </Polygon>
          ...
          <Marker position={[farm.latitude, farm.longitude]} icon={farmIcon}>
            <Popup>
              <div className="text-xs p-1 max-w-[220px]">
                ...
                <div className="mt-1.5 pt-1.5 border-t border-slate-200 text-[11px] text-slate-700 space-y-0.5">
                  <div>
                    <strong>Acreage:</strong> {farm.sizeAcres} Acres
                  </div>
                  <div>
                    <strong>Crop:</strong> {farm.crop.cropName}
                  </div>
                  <div>
            ...
            <Popup>
              <div className="text-xs p-1 max-w-[220px]">
                ...
                <div className="mt-1.5 pt-1.5 border-t border-slate-200 text-slate-700 space-y-0.5">
                  <div>
                    <strong>Location:</strong> {chc.village}, {chc.district}
                  </div>
                  <div>
                    <strong>Fleet:</strong> {chc.activeMachines} / {chc.totalMachines} Machines Active
                  </div>
                  <div>
                    <strong>Phone:</strong> {chc.contactPhone}
                  </div>
                </div>
              </div>
            </Popup>
              ...
              <Popup>
                <div className="text-xs p-1 max-w-[220px]">
                  ...
                  <div className="text-slate-500 font-mono text-[10px]">
                    {machine.identifier} ({machine.category})
                  </div>
                  ...
                  {telemetry && (
                    <div className="mt-1.5 pt-1.5 border-t border-slate-200 text-[10px] text-slate-600 space-y-0.5">
                      ...
                      <div>
                        Speed: {telemetry.speedKmh} km/h • Fuel: {telemetry.fuelLevelPercent}%
                      </div>
                      <div>
                        Burn Rate: {telemetry.fuelConsumptionRateLph} L/h • {telemetry.engineTemperatureC}°C
                      </div>
                    </div>
                  )}
                </div>
      ...
      {/* Map Legend Overlay positioned at bottom-left to avoid covering attribution */}
      <div className="absolute bottom-3 left-3 bg-white/95 backdrop-blur-md rounded-2xl p-3 shadow-elevated border border-slate-200/90 text-xs text-slate-700 space-y-1.5 z-[1000] pointer-events-auto">
